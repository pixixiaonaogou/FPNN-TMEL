...
class SeResNeXt50Unet(nn.Module):
    ...
    def __init__(self,dilation=False,edge_model=False):
        super().__init__()
    ...
    def criterion(self, logit, truth):
        ...
        # return loss
        loss = BCE_DICE_LOSS(logit, truth)
...
class MiniSeResNeXt50Unet(nn.Module):
    def load_pretrain(self, pretrain_file):
        pretrain_dict = torch.load(pretrain_file)
        state_dict = {}
        keys = list(pretrain_dict.keys())
    ...
    def __init__(self,dilation=False,edge_model=False):
       ...
       # self.decoder5 = Decoder(2048 + 512, 512, 64,OC=True,dilation=(2,4,6))
        self.decoder4 = Decoder(256 + 1024, 512, 64,OC=True,dilation=(4,8,12))
        self.decoder3 = Decoder(128 + 512, 128, 64)
        self.decoder2 = Decoder(64 + 256, 64, 64)
        self.decoder1 = Decoder(64, 32, 64)
    ...
    def forward(self, x):
       ...
       # ], 1)
        ...
        e4 = self.encoder4(e3)  # ; print('e4',e4.size())
       # e5 = self.encoder5(e4)  # ; print('e5',e5.size())
        f = self.center(e4)  #; print('center',f.size())
        if self.dilation:
            f1= self.center1(f)#; print('center',f1.size())
            f2=self.center2(f1)#; print('center',f2.size())
            # f3=self.center3(f2); print('center',f3.size())
        ...
        f = F.upsample(f, scale_factor=2, mode='bilinear', align_corners=True)
        #d4 = self.decoder5(torch.cat([f, e4], 1))  # ; print('d5',d5.size())
        #d4 = F.upsample(d4, scale_factor=2, mode='bilinear', align_corners=True)
...
class mini_non_local_network(nn.Module):
    def load_pretrain(self, pretrain_file):
        pretrain_dict = torch.load(pretrain_file)
        state_dict = {}
        keys = list(pretrain_dict.keys())
    ...
    def __init__(self,dilation=False,edge_model=False):
      ...
      #  )
        ...
        self.fusion =  afnb(256, 512, 256, 256, 256, dropout=0.05, sizes=([1]))
    ...
    def forward(self, x):
       ...
       # e4 = self.encoder4(e3)  # ; print('e4',e4.size())
       # e5 = self.encoder5(e4)  # ; print('e5',e5.size())
        f = self.max(self.fusion(e2,e3))  #; print('center',f.size())
        if self.dilation:
            f1= self.center1(f)#; print('center',f1.size())
            f2=self.center2(f1)#; print('center',f2.size())
            # f3=self.center3(f2); print('center',f3.size())
       ...
       # d4 = self.decoder4(torch.cat([f, e4], 1))  # ; print('d4',d4.size())
      #  d4 = F.upsample(d4, scale_factor=2, mode='bilinear', align_corners=True)
        d3 = self.decoder3(torch.cat([f, e3], 1))  # ; print('d3',d3.size())
        d3 = F.upsample(d3, scale_factor=2, mode='bilinear', align_corners=True)
        d2 = self.decoder2(torch.cat([d3, e2], 1))  # ; print('d2',d2.size())
        d2 = F.upsample(d2, scale_factor=2, mode='bilinear', align_corners=True)
        d1 = self.decoder1(d2)  # ; print('d1',d1.size())
...
class FPN(nn.Module):
    ...
    def __init__(self,dilation=False,edge_model=False):
     ...
     #   self.max = nn.MaxPool2d(kernel_size=2, stride=2)
       ...
       # self.decoder5 = Decoder(2048 + 512, 512, 64,OC=True,dilation=(2,4,6))
        self.decoder4 = Decoder(1024, 512, 256,OC=False,dilation=(4,8,12))
        self.decoder3 = Decoder(256 + 512, 256, 128)
        self.decoder2 = Decoder(128 + 256, 64, 64)
        self.decoder1 = Decoder(64, 32, 64)
...
#        x[:,2,:,:] = (x[:,2,:,:] - mean[2]) / std[2]
       ...
       # e5 = self.encoder5(e4)  # ; print('e5',e5.size())
        ...
        #f = F.upsample(f, scale_factor=2, mode='bilinear', align_corners=True)
        d4 = self.decoder4(e4)  # ; print('d4',d4.size())
        d4 = F.upsample(d4, scale_factor=2, mode='bilinear', align_corners=True)
        d3 = self.decoder3(torch.cat([d4, e3], 1))  # ; print('d3',d3.size())
        d3 = F.upsample(d3, scale_factor=2, mode='bilinear', align_corners=True)
        d2 = self.decoder2(torch.cat([d3, e2], 1))  # ; print('d2',d2.size())
...
def run_check_net():
   ...
   #net.load_pretrain(
    ...
    while i <= 500:
        ...
        i = i + 1
name_list = ['FPN','Unet','PSP','MSGRAP','SKNUnet','FPNN','FPNN-TMEL']
roc = [88.61,87.22,87.61,92.28,93.72,96.48,97.41]
pr  = [70.15,60.53,69.82,78.81,80.57,87.59,88.66]